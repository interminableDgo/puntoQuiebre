...
# === 2. ESTRATEGIA EXTENDIDA (Hasta 300 Usuarios) ===
class ExtendedStepShape(LoadTestShape):
    """
    Sube 10 usuarios cada 10 segundos.
    Meta teórica: 300 usuarios en 5 minutos.
    """
    step_time = 10  # Duración del escalón (10 segundos)
    step_load = 10  # Agregamos 10 usuarios por escalón (Más agresivo)
    spawn_rate = 5  # Nacen 5 por segundo (Tarda 2s en llenar el escalón)
...
# === 3. USUARIO OMNI DOCTOR ===
class OmniDoctor(HttpUser):
    host = f"http://{TARGET_IP}:5002"
    # Mantenemos 1 segundo de espera para generar buena presión
